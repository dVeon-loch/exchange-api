...
impl BinanceSpot {
    ...
    fn build_url_suffix(symbols: &[String], streams: &[StreamKind], update_rate: Option<UpdateRate>) -> String {
        ...
        let depth_speed_str = if depth_speed.as_millis() >= 1000 { "1000ms" } else { "100ms" };
    ...
    fn subscriptions(&self, symbols: &[&str], streams: &[StreamKind]) -> Vec<String> {
        ...
        for stream in streams {
            match stream {
                ...
                StreamKind::OrderBook { .. } => {
                    for symbol in symbols {
                        params.push(format!("{}@depth@100ms", symbol));
                    }
                }
                StreamKind::Ticker => {
                    for symbol in symbols {
    ...
    fn parse_stream(
        ...
        match msg.parse()? {
            CombinedStreamEvent::Ticker(ticker) => {
                Ok(Some(exchange_api::StreamData::Ticker(ticker.try_into()?)))
            }
            CombinedStreamEvent::Trade(trade) => {
                Ok(Some(exchange_api::StreamData::Trade(trade.try_into()?)))
            }
            CombinedStreamEvent::DepthUpdate(update) => {
                ...
                match ob.handle_update(&update) {
                    Ok(Some(snapshot)) => Ok(Some(exchange_api::StreamData::OrderBook(snapshot))),
                    Ok(None) => Ok(None),
                    Err(_) => {
                        ob.reset();
